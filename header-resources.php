...
<?php
  ...
  if (isset($_GET['location'])) {
    $location_name = sanitize_text_field($_GET['location']);
  } else {
  ...
  $locationArg = array(
    'post_type' => 'page',
    'posts_per_page' => 1,
    'pagename' => $location_name
  );
  $PostLocation = new WP_Query( $locationArg );

  if ( $PostLocation->have_posts() ) :
    while ( $PostLocation->have_posts() ) : $PostLocation->the_post();
  ...
  $opt = get_location_resrouces_page($location_name);
